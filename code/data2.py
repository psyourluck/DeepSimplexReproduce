...
def path_seeking(source, target, link):
    ...
    while True:
        ...
        # else:
        #     l = path[-1].size
        #     i = np.random.choice(np.arange(0,l))
        source = path[-1][0]
        if source in lis:
            path[-1] = np.delete(path[-1], 0)
            continue
...
# L = []
# for i in range(141):
#     L.append(path_seeking(50,(i+1),c))
...
for i in range(P):
    ...
    path_add = link_index(path_cont, c)
    if path_cont == []:
        continue
    # if path_cont[-1] == 50 & source != 50:
    #     ls = L[target-1]
    #     if ls == []:
    #         continue
    #     ls_add = link_index(ls, c)
    #     path_add = path_add + ls_add
    R[path_add,i] = 1
